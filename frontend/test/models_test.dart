...
void main() {
  group('Serialization', () {
    test(
        ...
        () async {
      final Category c1 =
          Category(id: 1, name: "test name", description: "test description");
      final c1Json = jsonEncode(c1.toJson());
      final c2 = Category.fromJson(json.decode(c1Json));
      expect(c1, c2);
    });

    test('Converting a Item to json an retrieving it should give the same Item',
        () async {
      final Item i1 = Item(
        id: 1,
        categoryId: 10,
    ...
    test(
        ...
        () async {
      final Brand b1 =
          Brand(id: 1, name: "test name", description: "test description");
